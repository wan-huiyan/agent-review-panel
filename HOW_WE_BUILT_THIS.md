...
Two identical panel runs on the same the causal client webapp (v2.10, `epic-poincare` and `thirsty-nobel` worktrees) produced only ~30% finding overlap. Each run missed a different P0 bug the other caught. A consistency analysis identified four root causes:
