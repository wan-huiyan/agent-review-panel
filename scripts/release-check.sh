...
#   4. The canonical version in `.claude-plugin/plugin.json` matches the version
...
#
# v3.0+ paths: post-PR-33 single-plugin layout — plugin.json at repo root,
# skills under skills/<name>/. The original PR #32 version of this script
# read from plugins/agent-review-panel/.../ — those paths were collapsed
# when the marketplace was reduced to one plugin.
...
CANON_VERSION="$(grep -E '"version"' .claude-plugin/plugin.json | head -1 | sed -E 's/.*"version"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/')"
PLUGIN_NAME="$(grep -E '"name"' .claude-plugin/plugin.json | head -1 | sed -E 's/.*"name"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/')"
...
# Flag any slash command in README that uses a plugin prefix other than the
# current canonical one. We extract every "/<word>:agent-review-panel" hit and
# compare against CORRECT_SLASH.
STALE_HITS=$(grep -nE '/[a-z0-9-]+:agent-review-panel' README.md 2>/dev/null \
  | grep -v "$CORRECT_SLASH" || true)
if [ -n "$STALE_HITS" ]; then
  fail "README.md contains slash commands that don't match canonical '$CORRECT_SLASH':"
  echo "$STALE_HITS" | sed 's/^/    /'
else
  pass "Slash commands in README.md consistent with '$CORRECT_SLASH'"
fi
...
check_version_in "skills/agent-review-panel/eval-suite.json" '"version"' "agent-review-panel eval-suite version"
check_version_in "skills/agent-review-panel/SKILL.md" '^# Agent Review Panel v' "SKILL.md h1 header"
