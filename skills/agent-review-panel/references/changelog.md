...
## v3.9.1 (2026-08-11) — The debate-in-Workflow recipe taught a silently degraded run

The worked example for running the panel under the Workflow/ultracode engine was three stages — round 1 → peers-injected round 2 → judge — and silently omitted Phases 4, 6, 7, 8, 9, 10, 11, 12, 13, 13.5, 14.5 and 15, while advertising that authoring a Debate phase satisfies the NO-DEBATE check. It taught the reader to satisfy the debate detector while skipping every verification pass. The recipe now declares itself a compressed run and must stamp a COMPRESSED RUN banner with a fixed phases-skipped list; it has five mandatory stages, where stage 3 is budget mode's existing consolidated verifier (Phases 8+10+11) and stage 5 is a terminal Report stage that IS Phase 15.1. Phase 15.1 also now documents the hole in its own chokepoint: a Workflow script ending at Judge never reaches it, so no banner check fires — unclosable by detection from inside the skill, mitigated authorially by the Report stage. Tests: 521 → 529. See root `CHANGELOG.md` for the full entry.

## v3.9.0 (2026-08-10) — Reviewer addressing contract + blocked-reviewer handling

Investigated adapting the panel to Claude Code's cross-session/peer messaging update (2.1.224). Two of the three changes here are pre-existing defects that update only made detectable. **Reviewer addressing contract:** persistent reviewers are now addressed by the `agentId` from their spawn result — the persona label and the Agent tool's `description` both fail to resolve against a nameless subagent, only the raw `agentId` works — and a `SendMessage` `{"success": false}` is treated as a failed agent under the existing retry-once rule. **Fresh-spawn fallback memory:** the Phase 4/5/7 prompt templates now tell a freshly spawned replacement to read its own prior `state/` files, closing a gap SKILL.md had promised five times but never implemented. **Blocked-reviewer handling:** a reviewer that cannot see the code now writes `state/reviewer_<slug>_BLOCKED.md` instead of its phase file, so the existing Phase 13.5 gate treats it as a missing phase rather than counting a well-formed "no findings" review as agreement. Agent teams and cross-session messaging were evaluated and declined — reasons recorded in `docs/plans/2026-08-10-peer-messaging-adaptation-design.md`. Reviewed by a four-reviewer adversarial panel (SOUND WITH AMENDMENTS); the review run itself was compressed, so every load-bearing claim was hand-verified, catching corrections across three claims in the design doc and two errors in the review. Tests: 499 → 521. See root `CHANGELOG.md` for the full entry.

## v3.4.0 (2026-06-04) — VoltAgent catalog expansion + drift automation

Roughly doubles the Signal-Detected Specialist Mapping coverage with 30 new content-signal→VoltAgent rows (Vue, Angular, Next.js, React Native, Electron, Django, FastAPI, Spring Boot, Symfony, C/C++, Kotlin, Elixir, MLOps, reinforcement learning, prompt optimization, AI systems, DBA, incident response, Windows, cloud security, CLI, MCP servers, refactoring, build systems, dependencies, git workflows, game dev, API docs, legal, UX research); 10 code-relevant rows mirrored into `plan-review-integrator`. Catalog count claim corrected from "127+" to "130+ across 10 families" (135 real agents). Adds drift automation so the hand-maintained mapping can't silently rot: a vendored `references/voltagent-catalog.json` snapshot (with provenance), `scripts/refresh-voltagent-catalog.sh` to regenerate it, `scripts/voltagent-catalog-check.sh` for on-demand checks, `tests/voltagent-catalog.test.mjs` as the CI gate (validates every live `voltagent-<family>:<slug>` resolves in the snapshot), and a new `release-check.sh` section 8. The check caught a pre-existing namespace mislabel on first run (`voltagent-biz:risk-manager` → `voltagent-domains:risk-manager`). "Automatic" here means availability (already live-detected in Phase 1) + drift *detection* — the editorial which-agent-for-which-signal choice stays human. Additive; 406/406 tests pass. See root `CHANGELOG.md` for the full entry.

## v3.3.0 (2026-05-14) — Live-State Claim Discipline (#40)

Adds a cross-cutting rule set for findings that assert facts about *live infrastructure or runtime state* (deployed IAM/IAP/auth config, running crons, production env vars, BigQuery partition keys, load-balancer routing). Motivated by a 2026-04-27 run where the panel read `echo "gcloud ..."` lines in deploy scripts — operator-facing documentation printed to the terminal — as live IAM config and raised a false P0 "IAM/IAP divergence" that survived all 5 reviewers, 3 debate rounds, and the Supreme Judge. Four rules: (1) distinguish declarative config from imperative documentation (`echo`/comment/usage lines are never evidence for a live-state claim) from live `describe`-class output; (2) tag live-state claims `[LIVE-VERIFIED]` or `[STATIC-INFERENCE]`, capping `[STATIC-INFERENCE]` at P1; (3) consensus does not compound on a shared artifact — same-lines agreement or cross-citation chains are tagged `[STATIC-INFERENCE-CONSENSUS]` and count as one source; (4) a pre-promotion falsification check before any P0. Wired into Phases 3, 5, 6, 11, 14; three new epistemic labels. Additive — no new phase, agent, or state file. See root `CHANGELOG.md` for the full entry. Note: the v3.2.0 entry (post-judge verification gate + Chart.js wrapper-div mandate, #41/#42) lives in the root `CHANGELOG.md`.
...
Motivated by a real consistency gap: two identical panel runs on the same the causal client webapp (v2.10) produced only ~30% finding overlap, each missing a different P0 bug. Root causes identified:
