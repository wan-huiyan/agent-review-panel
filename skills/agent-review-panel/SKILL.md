...
## The Work Under Review
{full_content — or a summary if very long}
...
**Still in dispute:**
1. {point} — {Side A: reviewers + argument} vs {Side B: reviewers + argument}
...
so agents focus on unresolved issues, and (2) it's the input for convergence
checking.
...
*Review conducted by Agent Review Panel v2 — methodology based on
...
in Phase 2 (independent review) and Phase 4.5 (completeness audit). Use
round summaries + key excerpts in debate rounds to avoid context overflow.
Always provide full content to the judge.
...
When auto-selecting personas, consider the work type:
...
- If the work contains SQL/data pipelines → include a "Data Quality Auditor"
- If it's user-facing → include a "UX/Accessibility Advocate"
- If it involves ML → include a "Statistical Rigor Reviewer"
- If it's infrastructure → include a "Reliability/SRE Perspective"
- Always include the Devil's Advocate — they prevent premature consensus
