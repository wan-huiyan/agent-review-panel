...
| v2.16.2 | 2026-04-08 | **Marketplace rename-back** (`plugin` → `agent-review-panel`, commit `00e2149`) + **plugin.json skills-field hotfix** (PR #24 — added `"skills": ["./"]` so SKILL.md at plugin root actually loaded after PR #18's canonical layout). **Plugin install name renamed** `agent-review-panel` → `roundtable` (commit `a522b00`); slash commands became `/roundtable:agent-review-panel`. **README unification** (PR #25) — unified shell-form and REPL-form install instructions with shell-form as primary. |
...
| v2.16.6 | 2026-04-19 | **Doc-drift sweep + release-check tooling** — self-review panel run on own repo surfaced 3 P0s (4× stale `/agent-review-panel:agent-review-panel` slash commands in README, wrong marketplace-name callout at README:129, CHANGELOG 2.16.5 entry omitting the roundtable rename + marketplace rename-back + phase-15.3 fix) and 4 P1s (test count claim 393→actual 354, HOW_WE_BUILT_THIS directory diagram documenting pre-v2.16.5 broken layout, missing `@plugin` migration block, ROADMAP frozen at v2.16.4). Fixed all; added `scripts/release-check.sh` to grep for prior-version strings + stale install commands across non-canonical files — prevents the recurring post-release doc-drift pattern. |
