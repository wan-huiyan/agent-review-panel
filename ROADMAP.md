...
| v2.16.1 | 2026-04-08 | **Multi-plugin marketplace bundle** (PR #22) — renamed marketplace to `plugin` (reverted to `agent-review-panel` at v2.16.2 after name-collision issues); bundled `plan-review-integrator` v2.0.0 as a second plugin in the same repo; moved `eval-suite.json` to per-plugin location; refactored `manifest-consistency` and `trigger-classification` tests to iterate all plugins with independent per-plugin cross-version checks. Test count: 308 → 352. |
| v2.16.2 | 2026-04-08 | **Marketplace rename-back** (`plugin` → `agent-review-panel`, commit `00e2149`) + **plugin.json skills-field hotfix** (PR #24 — added `"skills": ["./"]` so SKILL.md at plugin root actually loaded after PR #18's canonical layout). **Plugin install name renamed** `agent-review-panel` → `roundtable` (commit `a522b00`, reverted in v2.16.6); slash commands briefly became `/roundtable:agent-review-panel`. **README unification** (PR #25) — unified shell-form and REPL-form install instructions with shell-form as primary. |
...
| v2.16.4 | 2026-04-15 | **Phase 15.3 reliability fix** (PR #26) — sequential Phase 15 execution (15.1→15.2→15.3), disk-reading data strategy (agent reads files instead of orchestrator injection), mandatory verification gate, manual recovery path. Fixes silent HTML report generation failure. Process Overview diagram in SKILL.md synced to match sequential execution (PR #29). |
| v2.16.5 | 2026-04-19 | **Skills layout for Claude Code ≥2.1.112** (PR #30, first external contribution from @okuuva) — restructured to canonical nested layout (`plugins/<name>/skills/<name>/SKILL.md`), dropped `"skills"` field from `plugin.json` entirely. Fixes manifest validation failures on both `["./"]` ("Path escapes plugin directory") and `["SKILL.md"]` ("Invalid input"). Auto-discovery handles skill loading without any declaration. |
| v2.16.6 | 2026-04-19 | **Plugin name reverted** `roundtable` → `agent-review-panel` — collapses skill, plugin, and marketplace names to a single consistent `agent-review-panel` across all three layers. Install handle becomes `agent-review-panel@agent-review-panel`; slash command `/agent-review-panel:agent-review-panel`. **Breaking** for anyone installed under `@roundtable` (see Migration section in README). **Doc-drift sweep + release-check tooling** — self-review panel run on own repo surfaced 3 P0s (stale slash commands, wrong marketplace-name callout, incomplete CHANGELOG) and 4 P1s (test count 393→354, stale directory diagram, missing migration paths, ROADMAP frozen at v2.16.4). Fixed all; added `scripts/release-check.sh` to grep for prior-version strings + stale install commands across non-canonical files — prevents the recurring post-release doc-drift pattern. |

---

*This roadmap is maintained alongside the plugin at `plugins/agent-review-panel/skills/agent-review-panel/SKILL.md`. For the bundled `plan-review-integrator` plugin, see `plugins/plan-review-integrator/skills/plan-review-integrator/SKILL.md`.*
