...
| v3.2.0 | 2026-04-27 | **Post-judge verification gate + Chart.js wrapper-div mandate** ([#41](https://github.com/wan-huiyan/agent-review-panel/issues/41), [#42](https://github.com/wan-huiyan/agent-review-panel/issues/42)) — new Phase 14.5 re-verifies judge-introduced P0/P1 against ground truth via grep/Read/git; non-replicating findings get `[JUDGE-HALLUCINATED]` epistemic label and the verdict score is recomputed against the panel mean. Closes the v3.1.0 hole where the Supreme Judge could fabricate P0s in Step-0 verification (real 2026-04-27 reproduction: 12 phantom git-conflict markers in a clean file). Phase 15.3 spec now mandates `<div style="position: relative; height: NNNpx; width: 100%;">` around every Chart.js `<canvas>` to prevent the infinite-vertical-growth bug under `responsive: true` + `maintainAspectRatio: false`. |
| v3.3.0 | 2026-05-14 | **Live-State Claim Discipline** ([#40](https://github.com/wan-huiyan/agent-review-panel/issues/40)) — cross-cutting rule set for findings asserting facts about *live infrastructure or runtime state* (deployed IAM/IAP/auth, running crons, production env vars, BigQuery partition keys). Reviewers must distinguish declarative config from imperative documentation (`echo`/comment/usage lines are never evidence for a live-state claim) from live `describe`-class output; live-state claims are tagged `[LIVE-VERIFIED]` or `[STATIC-INFERENCE]` (the latter capped at P1); consensus does not compound on a shared artifact (`[STATIC-INFERENCE-CONSENSUS]` counts as one source); a pre-promotion falsification check gates every P0. Wired into Phases 3 / 5 / 6 / 11 / 14. Fixes the real 2026-04-27 false-positive P0 "IAM/IAP divergence" finding where the panel read deploy-script `echo` lines as live IAM config. |
| v3.4.0 | 2026-06-04 | **VoltAgent catalog expansion + drift automation** — 30 new content-signal→specialist rows in the SKILL.md mapping table (Vue, Angular, Next.js, React Native, Electron, Django, FastAPI, Spring Boot, Symfony, C++, Kotlin, Elixir, MLOps, RL, prompt-eng, DBA, incident response, CLI, MCP, refactoring, build, dependency, git, game, API docs, legal, UX, …); 10 mirrored into `plan-review-integrator`. Catalog count claim corrected 127+ → 130+ (135 agents). New drift automation: vendored `references/voltagent-catalog.json` snapshot + `refresh-voltagent-catalog.sh` + `voltagent-catalog-check.sh` + `tests/voltagent-catalog.test.mjs` (CI gate) + `release-check.sh` section 8 — every `voltagent-<family>:<slug>` in the live skill files must resolve to a real agent in the snapshot. The check caught a pre-existing mislabel on its first run (`voltagent-biz:risk-manager` → `voltagent-domains:risk-manager`). Additive; runtime availability still detected live with graceful fallback. |
| v3.5.0 | 2026-06-06 | **Loud debate-skip — `[NO-DEBATE]` banner** ([#56](https://github.com/wan-huiyan/agent-review-panel/issues/56)) — a 7-day audit (51 real runs) found the adversarial debate (Phase 5–7) executed in only 1; invoking the skill no longer reliably produced a debate (it silently collapsed to reviews→judge on inline/workflow shapes). Fix: detection anchored at the **Phase 15.1 report-write chokepoint** (every completed run passes it) — when zero `reviewer_*_phase_5_round1.md` state files exist, stamp a `[NO-DEBATE]` banner in the markdown report AND the HTML dashboard, suffix every action item `[NO-DEBATE]`, and cap verdict confidence at Medium. Phase 13.5 gains a panel-level debate-presence assertion (never proceed to judge with zero debate output silently). New **Review-Mode Spectrum** doc + **debate-in-Workflow recipe** (round 1 → peers-injected round 2 → judge) so ultracode runs can author debate explicitly. Mirrors the v3.1 `[COMPRESSED]` machinery; banners stack (NO-DEBATE first). |
| v3.6.0 | 2026-06-28 | **Assessment mode — quality discrimination + control-validation gate** — a fourth review mode for subjective-quality deliverables (strategy reports, marketing/creative copy, pitches, research syntheses), where the failure mode is **saturation** (a no-data control scores as high as a real report) rather than discrete defects. Adds: mode detection + a generated 4-archetype domain-adversarial persona set (Domain-Authenticity / Executability-Operator / Decision-Maker-ROI / Intent-Fidelity); **subtract-points + veto** scoring (a fatal domain violation → 0, not averaged away); the **swap test** (replace the subject's proper noun with a competitor's — if the strategy still reads true, zero specificity); an **out-of-band currency check** (personas can't tell a confident current fact from a confident stale one); and the signature **control-validation gate** — run a degenerate no-input control through the same personas and drop any that can't score it below ~3/10, testing the panel against a *known-bad floor* (complements the Phase-6 CONSENSAGENT agreement check). Origin: an external China brand-report bake-off where a 6-judge panel's four quality axes all saturated; the strongest models, asked to design their own panel, independently proposed these exact mechanisms. |
| v3.7.0 | 2026-07-16 | **Budget mode — measured-cost reduction profile** — explicit opt-in ("budget review" / `budget` arg) running the full adversarial shape at ~20–25% of full-run cost. Grounded in a 2026-07-16 token audit of a real full-protocol run ($162): the orchestrator main loop was **69% of cost** (157 turns × 270k→630k ctx), not the reviewer fan-out (7%). Cuts, in measured order: orchestrator turn diet (batched launches, persistent reviewers, ≤50-word returns, ≤25-turn target, fresh-session guidance); Phases 3+4 merged; exactly 1 debate round; Phases 8+10+11 → ONE consolidated sonnet verifier; 12a-only; 13 skipped; single opus judge; 14.5 as orchestrator grep-check; 15.1 markdown-only. `[BUDGET-MODE]` banner + stacking order; multi-run/deep/assessment incompatibilities declared. Tests: 443 → 484. |
| v3.7.1 | 2026-07-16 | **Budget mode discoverability** — broader named triggers ("budget panel", "affordable review", "token-efficient review", "frugal review", "lite panel") + cost-constraint trigger (any panel request with explicit cost/token concern opts in, confirmed in one line, overridable with "full panel"); README "How to trigger" + "why opt-in, not default" docs; +2 eval trigger cases. No protocol changes. |
| v3.8.0 | 2026-07-16 | **Orchestrator Efficiency Discipline — turn diet by default (all modes)** — the quality-free portion of budget mode's turn diet becomes the default for every mode: batched launches, persistent reviewers (one spawn per persona, Phases 4/5/7 via SendMessage, fresh-spawn fallback), ≤50-word agent returns (was 100), no inter-phase narration, ≤40-turn full-panel target (measured pre-discipline: 157 turns = 69% of cost), fresh-session recommendation, state-file reads only for orchestrator logic. No review/debate/verification work removed. Budget mode keeps its stricter deltas (≤25 turns, strictest read rule) + quality-trading cuts. |
| v3.8.1 | 2026-08-04 | **Trigger recovery — the description was over the listing cap** — Claude Code caps each skill's listing entry at `skillListingMaxDescChars` (1536) and truncates the overflow with no phrase-boundary awareness. At 2,703 chars, **25 of 51 trigger phrases were past the cut and could never fire** — including *every* budget-mode trigger. Root cause traced to v2.14 (1,501 → 2,004 chars); v3.7.0 and v3.7.1 then built on a cliff, so v3.7.1's entire discoverability payload was born dead. Description rewritten to 1,505 chars with zero triggers lost (synonym runs compressed, not deleted); measured against the 39 positive eval prompts: 12 better / 27 same / 0 worse, mean coverage 0.4950 → 0.5673, positive-negative separation +0.2605 → +0.3183 (corrected 2026-08-05 to the committed harness; the original 13 / 25 / 1 and 49.5% → 56.9% / +26.0 → +32.0 came from an uncommitted one-off and do not reproduce — see CHANGELOG v3.8.1). `scripts/check_skill_descriptions.py` + `tests/description-cap.test.mjs` gate it in CI (verified failing on v3.8.0, passing here). No protocol changes. |
| v3.8.2 | 2026-08-05 | **Accuracy — the v3.8.1 coverage table did not reproduce** — the cited figures came from an uncommitted one-off; the committed `scripts/score_trigger_coverage.py` reports 12 better / 27 same / 0 worse, 0.4950 → 0.5673 mean coverage, +0.2605 → +0.3183 separation. Corrected in place with an erratum, reproduce command pinned to commits. Also commits the `sample-report-budget-mode` golden snapshot (the fixture was tracked, the snapshot was not, so the test auto-created it and asserted nothing) and re-vendors the description gate at context-police v2.2.1. No protocol changes. |
| v3.8.3 | 2026-08-05 | **Vendored gate re-pinned at context-police v2.3.0** — wrap corruption is now scored over disabled skills too (no effect here; this repo has none), and a new `NO HEADROOM` tier separates descriptions with under 40 chars to spare from the broad `APPROACHING CAP` bucket. This skill sits at 1,505 chars — 31 to spare — so it now surfaces there. No protocol changes. |
| v3.9.0 | 2026-08-10 | **Reviewer addressing contract + blocked-reviewer handling** — persistent reviewers are addressed by the `agentId` from their spawn result, not the persona name or Agent-tool `description` (neither resolves against a nameless subagent), and a `SendMessage` `{"success": false}` is now treated as a failed agent under the existing retry-once rule. The fresh-spawn fallback gains the one preamble line that makes it actually read its own prior `state/` files, closing a gap SKILL.md had promised five times but `prompt-templates.md` never implemented. A reviewer that could not see the code now writes `state/reviewer_<slug>_BLOCKED.md` instead of its phase file, so the existing Phase 13.5 gate treats it as a missing phase rather than a well-formed "no findings" vote. Investigated adapting to Claude Code's peer-messaging update (2.1.224); agent teams and cross-session messaging were evaluated and declined, with reasons recorded in `docs/plans/2026-08-10-peer-messaging-adaptation-design.md`. Tests: 499 → 521. |
| v3.9.1 | 2026-08-11 | **The debate-in-Workflow recipe taught a silently degraded run** — the v3.5.0 worked example (round 1 → peers-injected round 2 → judge) was presented as the way to get the panel's depth under Workflow/ultracode, but omitted Phases 4, 6, 7, 8, 9, 10, 11, 12, 13, 13.5, 14.5 and 15 — while explicitly advertising that authoring a `Debate` phase *"satisfies the NO-DEBATE check"*, i.e. teaching the reader to pass the debate detector with the verification machinery switched off. A 2026-08-10 run authored from it shipped two wrong findings, one per omitted defect class (a changelog citation to the wrong Claude Code version → Phase 10; a P0 that one read-only probe falsified → Phase 11). Fix is banner-first, since SKILL.md loads into orchestrator context every run: the recipe declares itself a compressed run and MUST stamp a `COMPRESSED RUN` banner with a fixed phases-skipped list (no new banner type), and grows from three mandatory stages to **five** — buying back budget mode's existing consolidated verifier (Phases 8+10+11, one `agent()` call, the stage that catches both defects) and a terminal `Report` stage that IS Phase 15.1. Phase 15.1 now documents the hole in its own chokepoint: a Workflow script ending at `Judge` never reaches it, so no banner check fires — unclosable by detection, mitigated authorially. Tests 521 → 529. |
